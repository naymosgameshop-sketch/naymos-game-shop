    ...
    <div className="w-full space-y-6 sm:space-y-8">
      ...
      {step === 1 && <div className="space-y-6">
        {(game.provider_availability && game.provider_availability !== 'available') && (
          <div className="rounded-2xl border border-rose-200 bg-rose-50/90 p-4 text-rose-800 text-xs sm:text-sm font-semibold flex items-center gap-3">
            <AlertCircle className="w-5 h-5 text-rose-600 shrink-0" />
            <div>
              <div className="font-bold text-rose-900">ขณะนี้ระบบเติมเกมนี้ไม่พร้อมให้บริการชั่วคราว</div>
              <div className="text-xs text-rose-700 mt-0.5">{game.provider_error_message || 'ระบบ Provider ต้นทางปิดปรับปรุงหรือไม่มีสต็อก สินค้าจะไม่สามารถทำรายการสั่งซื้อได้ในขณะนี้'}</div>
            </div>
          </div>
        )}
        ...
        <div className="bg-white rounded-2xl border border-sky-100 p-4 sm:p-6 shadow-xs"><div className="flex items-center justify-between mb-3 sm:mb-4"><h3 className="text-sm sm:text-base font-bold text-slate-800">เลือกแพ็กเกจ (สามารถเลือกได้หลายแพ็ก)</h3>{totalPackagesCount > 0 && <span className="text-xs font-semibold px-2.5 py-0.5 rounded-full bg-sky-100 text-sky-700">เลือกแล้ว {totalPackagesCount} แพ็ก</span>}</div><div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">{products.map((p: any) => {
              const item = cartItems.find((x) => x.productId === p.id && cartItemKey(x) === cartItemKey({ productId: p.id, playerData: activePlayerData }));
              const qty = item?.quantity || 0;
              const price = getProductPrice(p);
              const isOutOfStock = p.availability === 'out_of_stock' || (p.stock !== null && p.stock !== undefined && p.stock <= 0);
              const isProviderError = p.availability === 'provider_error';
              const isUnavailable = !p.is_active || p.availability === 'unavailable' || isOutOfStock || isProviderError;
              const stockLabel = isOutOfStock ? 'สินค้าหมด' : isProviderError ? 'ระบบขัดข้อง' : !p.is_active ? 'ปิดจำหน่าย' : p.availability === 'unavailable' ? 'ไม่พร้อมจำหน่าย' : null;

              return <div key={p.id} className={`relative p-3.5 rounded-xl border transition-all flex flex-col justify-between ${
                isUnavailable
                  ? 'border-slate-200 bg-slate-100/70 opacity-60 grayscale cursor-not-allowed'
                  : qty > 0
              ...
              }`}><div><div className="flex items-start justify-between gap-2"><span className="font-bold text-xs sm:text-sm text-slate-800 leading-snug">{p.name}</span>{qty > 0 && <span className="w-5 h-5 rounded-full bg-sky-500 text-white flex items-center justify-center text-[10px] shrink-0"><Check className="w-3 h-3" /></span>}</div><div className="mt-1 flex items-baseline gap-1.5 flex-wrap">
