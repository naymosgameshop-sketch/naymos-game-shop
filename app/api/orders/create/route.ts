...
export async function POST(request: Request) {
  ...
  try {
    ...
    const result = await createMultiItemOrder({ userId: user.id, items: Array.isArray(body.items) ? body.items : [], couponCode: body.couponCode, pointsToUse: body.pointsToUse });
